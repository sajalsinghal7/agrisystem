...
//			Stream<String> line  = Files.lines(path).parallel();
	...
	public List<Stress> getAllDegreeDayForRegion(String regionOfAnalysis) {
		...
		for (Map<String, AttributeValue> item : result.getItems()) {
			try {
				   resultList.add(new Stress(item.get("id").getS(), Integer.valueOf(item.get("date").getN().trim()), Double.valueOf(item.get("eto").getN()), Double.valueOf(item.get("eta").getN()), Double.valueOf(item.get("etc").getN()), item.get("regionOfAnalysis").getS()));
			} catch(Exception e) {
				e.printStackTrace();
			}
		 }
